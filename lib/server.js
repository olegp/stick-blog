...
app.get("/sitemap.xml", function(request) {
  return app.render("blog-sitemap.xml", app.renderParams({
    posts: app.db.posts.find({}, {slug:1}).sort({created:-1}).toArray().map(function(post) {
      return post;
    })
